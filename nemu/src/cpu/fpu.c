...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		...
		if(exp >= 0xff) {
			...
			//printf("\e[0;31mPlease implement me 2 at fpu.c\e[0m\n");
			if(sign==0)
				return p_inf.val;
			else
				return n_inf.val;
		...
		if(exp == 0) {
			...
			sig_grs=sig_grs>>1;
			sig_grs |= sticky;
			//printf("\e[0;31mPlease implement me 3 at fpu.c\e[0m\n");
			//assert(0);
		}
		if(exp < 0) { 
			...
			if(sign==0)
				...
				return n_zero.val;
			//printf("\e[0;31mPlease implement me 4 at fpu.c\e[0m\n");
			//assert(0);
			overflow = true;
		}
		...
		while(((sig_grs >> (23 + 3)) == 0) && exp > 0) {
			...
			sig_grs=sig_grs<<1;
			exp=exp-1;
			//printf("\e[0;31mPlease implement me 5 at fpu.c\e[0m\n");
			//assert(0);
		}
		if(exp == 0) {
			// denormal
	...
	if(!overflow) {
		...
		else if(test_grs==4)
		{
			...
			sig_grs=sig_grs>>3;
		//printf("\e[0;31mPlease implement me 6 at fpu.c\e[0m\n");
		//assert(0);
	}
